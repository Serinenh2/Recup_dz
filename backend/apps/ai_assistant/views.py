...
def detecter_anomalies_bsd(utilisateur=None):
    anomalies = []
    aujourd_hui = timezone.now().date()
    bsds = BordereauSuiviDechet.objects.all()
    for bsd in bsds:
        if bsd.statut == 'BROUILLON' and bsd.date_creation:
            date_creation = bsd.date_creation.date() if hasattr(bsd.date_creation, 'date') else bsd.date_creation
            if (aujourd_hui - date_creation).days > 7:
                anomalies.append({
                    'type': 'bsd_incomplet',
                    'niveau': 'warning',
        ...
        if bsd.date_emission and bsd.statut not in ['TERMINE', 'ANNULE', 'ARCHIVE']:
            if (aujourd_hui - bsd.date_emission).days > 14:
                anomalies.append({
                    'type': 'bsd_retard',
                    'niveau': 'danger',
...
class AIConversationViewSet(viewsets.ModelViewSet):
    ...
    def _generer_reponse_intelligente(self, message, conversation, contexte_supp):
        ...
        if any(mot in msg_lower for mot in ['déchet', 'déchets', 'nomenclature', 'code', 'classification']):
            return self._rechercher_nomenclature(message)
        if any(mot in msg_lower for mot in ['agrément', 'agréments', 'validité']):
            return self._verifier_agrements()
        if any(mot in msg_lower for mot in ['bsd', 'bordereau', 'bordereaux']):
            return self._analyser_bsd()
        if any(mot in msg_lower for mot in ['stock', 'stocks', 'quantité']):
            return self._analyser_stocks()
        if any(mot in msg_lower for mot in ['loi', 'loi 01-19', 'décret', 'réglementation']):
            return self._recherche_reglementaire(message)
        if any(mot in msg_lower for mot in ['bonjour', 'salut', 'hello', 'aide', 'help']):
            return {
...
Comment puis-je vous aider aujourd'hui ?"""
    ...
    def _analyser_bsd(self):
        ...
        for bsd in bsds:
            date_creation = bsd.date_creation.date() if hasattr(bsd.date_creation, 'date') else bsd.date_creation
            if bsd.statut == 'BROUILLON' and date_creation and (aujourd_hui - date_creation).days > 7:
...
class AIAlertViewSet(viewsets.ModelViewSet):
    ...
    def generer_alertes_auto(self, request):
        anomalies_bsd = detecter_anomalies_bsd(request.user)
