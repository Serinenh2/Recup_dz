...
class UserSerializer(serializers.ModelSerializer):
    ...
    tenant_nom        = serializers.SerializerMethodField()

    class Meta:
        model  = User
        fields = ['id','username','email','first_name','last_name',
                  'role','role_display','phone','wilaya','is_superuser',
                  'recuperateur_id','recuperateur_nom','tenant_nom']
    ...
    def get_tenant_nom(self, obj):
        try:
            return obj.tenant.nom if obj.tenant else None
        except Exception:
            return None
